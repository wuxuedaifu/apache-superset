      ...
      case 'pie':
        ...
        if (fd.pie_label_type === 'percent' || fd.pie_label_type === 'key_percent') {
          ...
          if (fd.pie_label_type === 'key_percent') {
            chart.labelType(d => `${d.data.x}: ${((d.data.y / total) * 100).toFixed(2)}%`);
          }
        }

        //DF add percentage and value
        if (fd.pie_label_type === 'key_percent_value' || fd.pie_label_type === 'key_value_percent'
            || fd.pie_label_type === 'value_percent') {
          let total = 0;
          data.forEach((d) => {
            total += d.y;
          });
          chart.tooltip.valueFormatter(d => `${((d / total) * 100).toFixed()}%`);
          if (fd.pie_label_type === 'key_percent') {
            chart.labelType(d => `${d.data.x}: ${formatter(((d.data.y / total) * 100))}%`);
          } else if (fd.pie_label_type === 'key_percent_value') {
            chart.labelType(d => `${d.data.x}: ${((d.data.y / total) * 100).toFixed(2)}%: ${formatter(d.data.y)}`);
          } else if (fd.pie_label_type === 'key_value_percent') {
            chart.labelType(d => `${d.data.x}: ${formatter(d.data.y)}: ${((d.data.y / total) * 100).toFixed(2)}%`);
          } else if (fd.pie_label_type === 'value_percent') {
            chart.labelType(d => `${formatter(d.data.y)}: ${((d.data.y / total) * 100).toFixed(2)}%`);
          }
          ;
        }
