...
METRIC_KEYS = [
    'metric', 'metrics', 'percent_metrics', 'metric_2', 'metrics_2', 'secondary_metric',
    'x', 'y', 'size',
]
...
class echarts_mixlinebarViz(NVD3Viz):

    """A simple line bar chart with dual axis"""

    viz_type = 'echarts_mixlinebar'
    verbose_name = _('Time Series - echarts_mix line bar Chart')
    sort_series = False
    is_timeseries = True

    def query_obj(self):
        d = super(echarts_mixlinebarViz, self).query_obj()
        m1 = self.form_data.get('metrics')
        m2 = self.form_data.get('metrics_2')
        d['metrics'] = [m1, m2]
        if not m1:
    ...
    def to_series(self, df, classed=''):
        ...
        for i, m in enumerate(metrics):
            if isinstance(m, list):
                ...
                index_list.append(i)
        metrics_control_1_num = index_list.count(index_list[0]) - 1
        ...
        for i, m in enumerate(metrics_list):
            ...
            d = {
                ...
                'classed': classed,
                'values': [
                    {'x': str(ds), 'y': ys[ds] if ds in ys else None}
                    for ds in df.index
                ],
                'strokeWidth': 2.5,
                'yAxis': 0 if i <= metrics_control_1_num else 1,
                'type': 'bar' if i <= metrics_control_1_num else 'line',  ###这个地方与dual line不同
            }
            chart_data.append(d)
        return chart_data
    ...
    def get_data(self, df):
        ...
        control_1 = fd.get('metrics')
        control_2 = fd.get('metrics_2')

        metrics_list = []
        metrics_2_list = []
        metrics_total_list = []

        if isinstance(control_1, list):
            for c1 in control_1:
                metrics = self.get_metric_label(c1)
                metrics_list.append(metrics)
        else:
            metrics = self.get_metric_label(control_1)
            metrics_list.append(metrics)

        if isinstance(control_2, list):
            for c2 in control_2:
                metrics_2 = self.get_metric_label(c2)
                metrics_2_list.append(metrics_2)
        else:
            metrics_2 = self.get_metric_label(control_2)
            metrics_2_list.append(metrics_2)

        metrics_total_list = metrics_list + metrics_2_list

        df = df.pivot_table(
            index=DTTM_ALIAS,
            values=metrics_total_list)
...
class echarts_stackbarlineViz(NVD3Viz):

    """A simple line bar chart with dual axis"""

    viz_type = 'echarts_stackbarline'
    verbose_name = _('Time Series - echarts_stack bar line Chart')
    sort_series = False
    is_timeseries = True

    def query_obj(self):
        d = super(echarts_stackbarlineViz, self).query_obj()
        m1 = self.form_data.get('metrics')
        m2 = self.form_data.get('metrics_2')
        d['metrics'] = [m1, m2]
        if not m1:
...
class echarts_3DLineViz(BaseViz):

    """Echarts 3D Line， mainly used for drawing waterfall plot of vibration spectrum"""

    viz_type = 'echarts_3DLine'
    verbose_name = _('Echarts 3D Line')
    sort_series = False
    is_timeseries = False

    def query_obj(self):
        d = super(echarts_3DLineViz, self).query_obj()
        fd = self.form_data
        d['metrics'] = [fd.get('metrics')]
        return d
    ...
    def to_series(self, df, classed=''):
        ...
        for i in range(len(df)):
            legend = 'RPM: ' + str(round(float(df['RPM'][i]), 2))
            wave_list = df['WaveList'][i].replace('\'','').strip('\r\n').split('\\n')
            wave_list = [x.split('\\t') for x in wave_list][:-1]
            # xAxis = [x[0] for x in wave_list]
            # zAxis = [x[1] for x in wave_list]
            zAxis = [[x[0], legend, float(x[1])] for x in wave_list]
            d = {
                'legend': legend,
                'zAxis': zAxis
            }
    ...
    def get_data(self, df):
        ...
        metrics_total_list = metrics_list
...
class echarts_baseLineViz(BaseViz):

    """Echarts base Line， mainly used for drawing FFT frequency spectrum plot"""

    viz_type = 'echarts_baseLine'
    verbose_name = _('Echarts base Line')
    sort_series = False
    is_timeseries = False

    def query_obj(self):
        d = super(echarts_baseLineViz, self).query_obj()
        fd = self.form_data
        d['metrics'] = [fd.get('metrics')]
        return d
    ...
    def to_series(self, df, classed=''):
        ...
        for i in range(len(df)):
            ...
            wave_list = [x.split('\\t') for x in wave_list][:-1]
            xAxis = [x[0] for x in wave_list]
            yAxis = [x[1] for x in wave_list]
            fft_y = np.fft.fft(yAxis)
            abs_y = np.abs(fft_y).tolist()  # 取复数的绝对值，即复数的模(双边频谱)
            angle_y = np.angle(fft_y).tolist()  # 取复数的角度
