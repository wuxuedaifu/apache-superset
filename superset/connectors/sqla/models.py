...
class SqlaTable(Model, BaseDatasource):
    ...
    def get_query_str(self, query_obj):
        ...
        if self.datasource_name == 'vibrationsdetail':
            sql = sql[0:sql.index('ORDER BY')]
        logging.info(sql)
    ...
    def get_sqla_query(  # sqla
        ...
        for m in metrics:
            if isinstance(m, list):
                for mm in m:
                    if utils.is_adhoc_metric(mm):
                        metrics_exprs.append(self.adhoc_metric_to_sa(mm, cols))
                    elif mm in metrics_dict:
                        metrics_exprs.append(metrics_dict.get(mm).sqla_col)
                    else:
                        raise Exception(_("Metric '{}' is not valid".format(mm)))
            else:
...
##DF TODO
    def get_sqla_query_vibrationdetail(  # sqla
            self,
            groupby, metrics,
            granularity,
            from_dttm, to_dttm,
